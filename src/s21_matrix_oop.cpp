...
S21Matrix::S21Matrix()
    : rows_(3), columns_(3), matrix_(new double[rows_ * columns_]) {
...
S21Matrix::S21Matrix(int rows, int cols)
    : rows_(rows), columns_(cols), matrix_(nullptr) {
...
// Copy constructor.
// It allocates a new matrix and copy values from other matrix.
S21Matrix::S21Matrix(const S21Matrix& other)
    : rows_(other.rows_), columns_(other.columns_), matrix_(nullptr) {
  if (other.matrix_ != nullptr) {
    matrix_ = new double[rows_ * columns_];
    copy_matrices(*this, other, rows_, columns_);
  }
}
...
S21Matrix& S21Matrix::operator=(const S21Matrix& other) {
  if (this != &other) {
    if (matrix_ == nullptr ||
        (rows_ * columns_ != other.rows_ * other.columns_)) {
      delete[] matrix_;
...
int S21Matrix::get_columns() const { return columns_; }
...
void S21Matrix::set_rows(const int& new_rows) {
  ...
  if (rows_ != new_rows) {
    ...
    copy_matrices(new_matrix, *this, limit, columns_);
...
bool S21Matrix::eq_matrix(const S21Matrix& other) const {
  if (rows_ == other.rows_ && columns_ == other.columns_) {
    for (int i = 0; i < rows_ * columns_; ++i) {
      if (std::abs(matrix_[i] - other.matrix_[i]) >
...
double& S21Matrix::operator()(int i, int j) {
  if (matrix_ == nullptr) {
    throw std::invalid_argument(
        "calling a bad allocated or an already deleted object");
  }
...
int S21Matrix::index(int i, int j) const { return i + j + i * (columns_ - 1); }
...
void S21Matrix::copy_matrices(S21Matrix& matrix1, const S21Matrix& matrix2,
                              int rows, int cols) {
